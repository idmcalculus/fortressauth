...
export default async function QuickStartPage({
  ...
  return (
    <article>
      ...
      <pre><code>{`import { SqlAdapter } from '@fortressauth/adapter-sql';
...
await adapter.migrate();`}</code></pre>
...
await adapter.migrate();`}</code></pre>
      ...
      <pre><code>{`import { FortressAuth, ConsoleEmailProvider } from '@fortressauth/core';
...
const fortress = new FortressAuth({
  ...
  config: {
    ...
    lockoutDurationMs: 15 * 60 * 1000, // 15 minutes
      ...
      <pre><code>{`import { ResendEmailProvider } from '@fortressauth/server';
...
const fortress = new FortressAuth({
  ...
  config: {
    // ... your config
      ...
      <pre><code>{`const result = await fortress.signUp({
...
if (result.success) {
  ...
  switch (result.error) {
    ...
    default:
      ...
      <pre><code>{`const result = await fortress.signIn({
  email: 'user@example.com',
  password: 'securePassword123',
  ipAddress: '127.0.0.1',
});
...
if (result.success) {
  ...
  switch (result.error) {
    ...
    default:
      ...
      <pre><code>{`// Get the token from the cookie
...
if (result.success) {
  ...
  return res.status(401).json({ error: result.error });
      ...
      <pre><code>{`// The token comes from the URL query parameter
...
if (result.success) {
  ...
  switch (result.error) {
    ...
    case 'EMAIL_VERIFICATION_EXPIRED':
      ...
      <pre><code>{`// Step 1: Request password reset
...
if (resetResult.success) {
  ...
  console.error('Password reset failed:', resetResult.error);
      ...
      <pre><code>{`const token = req.cookies.session;
...
res.json({ success: true });`}</code></pre>
      ...
      <ul>
        <li><a href="/docs/api/fortress-auth">API Reference</a> - Complete method documentation</li>
        <li><a href="/docs/api/configuration">Configuration Options</a> - Customize behavior</li>
        <li><a href="/docs/security/rate-limiting">Rate Limiting</a> - Protect against brute force</li>
        <li><a href="/docs/sdks/react">React SDK</a> - Frontend integration</li>
        <li><a href="/docs/deployment/docker">Docker Deployment</a> - Production setup</li>
      </ul>
    </article>
  );
}
