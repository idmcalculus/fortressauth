...
import { Book, ChevronRight, Code, Home, Menu, Rocket, Search, X } from 'lucide-react';
...
export default function DocsLayout({ children }: { children: React.ReactNode }) {
  ...
  const toggleSection = useCallback((sectionHref: string) => {
    setExpandedSections((prev) =>
      prev.includes(sectionHref) ? prev.filter((s) => s !== sectionHref) : [...prev, sectionHref],
    );
  }, []);
  ...
  const filteredNavItems = useMemo(() => {
    ...
    return navItems
      .map((section) => ({
        ...section,
        children: section.children?.filter((item) => item.title.toLowerCase().includes(query)),
      }))
      .filter(
        (section) =>
