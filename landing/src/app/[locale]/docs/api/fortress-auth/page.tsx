...
const fortress = new FortressAuth(
  ...
  config?: FortressConfigInput
      ...
      <pre><code>{`interface SignUpInput {
  email: string;
  password: string;
  ipAddress?: string;
  userAgent?: string;
...
// Error: { success: false, error: AuthErrorCode }`}</code></pre>

      <h4>Possible Errors</h4>
      <ul>
        <li><code>EMAIL_EXISTS</code> - Email already registered</li>
        <li><code>PASSWORD_TOO_WEAK</code> - Password doesn&apos;t meet requirements</li>
        <li><code>RATE_LIMIT_EXCEEDED</code> - Too many signup attempts</li>
        <li><code>INVALID_INPUT</code> - Invalid email format or input</li>
      </ul>

      <h3>signIn(input)</h3>
      <p>{t('api.fortressAuth.signInDesc')}</p>
      <pre><code>{`interface SignInInput {
  email: string;
  password: string;
  ipAddress?: string;
  userAgent?: string;
...
// Returns: Result<AuthResult, AuthErrorCode>`}</code></pre>

      <h4>Possible Errors</h4>
      <ul>
        <li><code>INVALID_CREDENTIALS</code> - Wrong email or password</li>
        <li><code>ACCOUNT_LOCKED</code> - Account locked due to failed attempts</li>
        <li><code>EMAIL_NOT_VERIFIED</code> - Email not yet verified</li>
        <li><code>RATE_LIMIT_EXCEEDED</code> - Too many login attempts</li>
      </ul>

      <h3>validateSession(rawToken)</h3>
      <p>{t('api.fortressAuth.validateSessionDesc')}</p>
      <pre><code>{`const result = await fortress.validateSession(token);
...
if (result.success) {
  ...
  console.log('Session expires:', session.expiresAt);
      ...
      <h4>Possible Errors</h4>
      <ul>
        <li><code>SESSION_INVALID</code> - Token is malformed or doesn&apos;t exist</li>
        <li><code>SESSION_EXPIRED</code> - Session has expired</li>
      </ul>

      <h3>signOut(rawToken)</h3>
      <p>{t('api.fortressAuth.signOutDesc')}</p>
      <pre><code>{`const result = await fortress.signOut(token);

// Returns: Result<void, AuthErrorCode>`}</code></pre>

      <h4>Possible Errors</h4>
      <ul>
        <li><code>SESSION_INVALID</code> - Token is invalid</li>
      </ul>

      <h3>verifyEmail(token, context?)</h3>
      <p>{t('api.fortressAuth.verifyEmailDesc')}</p>
      <pre><code>{`const result = await fortress.verifyEmail(token, {
  ipAddress: '127.0.0.1',
  userAgent: 'Mozilla/5.0...',
});
...
// Returns: Result<void, AuthErrorCode>`}</code></pre>
      ...
      <h4>Possible Errors</h4>
      <ul>
        <li><code>EMAIL_VERIFICATION_INVALID</code> - Token is invalid</li>
        <li><code>EMAIL_VERIFICATION_EXPIRED</code> - Token has expired</li>
        <li><code>RATE_LIMIT_EXCEEDED</code> - Too many verification attempts</li>
      </ul>

      <h3>requestPasswordReset(email)</h3>
      <p>{t('api.fortressAuth.requestPasswordResetDesc')}</p>
      <pre><code>{`const result = await fortress.requestPasswordReset('user@example.com');

// Returns: Result<void, AuthErrorCode>
// Always returns success to prevent email enumeration`}</code></pre>
      ...
      <pre><code>{`interface ResetPasswordInput {
  token: string;
  newPassword: string;
  ipAddress?: string;
  userAgent?: string;
...
// Returns: Result<void, AuthErrorCode>`}</code></pre>
      ...
      <h4>Possible Errors</h4>
      <ul>
        <li><code>PASSWORD_RESET_INVALID</code> - Token is invalid</li>
        <li><code>PASSWORD_RESET_EXPIRED</code> - Token has expired</li>
        <li><code>PASSWORD_TOO_WEAK</code> - New password doesn&apos;t meet requirements</li>
        <li><code>RATE_LIMIT_EXCEEDED</code> - Too many reset attempts</li>
      </ul>

      <div className="note">
        <strong>Note:</strong> A successful password reset invalidates all existing sessions 
        for the user, requiring them to sign in again.
      </div>

      <h3>getConfig()</h3>
      <p>{t('api.fortressAuth.getConfigDesc')}</p>
      <pre><code>{`const config = fortress.getConfig();

// Returns: Readonly<FortressConfig>
console.log(config.session.ttlMs);
console.log(config.password.minLength);`}</code></pre>

      <h2>Type Definitions</h2>

      <h3>AuthResult</h3>
      <pre><code>{`interface AuthResult {
  user: User;
  token: string;
}`}</code></pre>
