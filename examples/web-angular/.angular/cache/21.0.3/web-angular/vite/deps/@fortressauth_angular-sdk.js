import {
  ...
  Subject,
  Subscription,
} from './chunk-AWBSFIDB.js';

// ../../node_modules/.pnpm/@angular+core@19.2.17_rxjs@7.8.2_zone.js@0.15.1/node_modules/@angular/core/fesm2022/untracked-BKcld_ew.mjs
...
// ../../node_modules/.pnpm/@angular+core@19.2.17_rxjs@7.8.2_zone.js@0.15.1/node_modules/@angular/core/fesm2022/primitives/di.mjs
...
// ../../node_modules/.pnpm/@angular+core@19.2.17_rxjs@7.8.2_zone.js@0.15.1/node_modules/@angular/core/fesm2022/primitives/signals.mjs
var NOOP_CLEANUP_FN = () => {};
var WATCH_NODE = (() => {
  return __spreadProps(__spreadValues({}, REACTIVE_NODE), {
...
// ../../node_modules/.pnpm/@angular+core@19.2.17_rxjs@7.8.2_zone.js@0.15.1/node_modules/@angular/core/fesm2022/primitives/event-dispatch.mjs
var Attribute = {
  /**
...
// ../../node_modules/.pnpm/@angular+core@19.2.17_rxjs@7.8.2_zone.js@0.15.1/node_modules/@angular/core/fesm2022/core.mjs
...
function getInjectorDef(type) {
  return type && (Object.hasOwn(type, NG_INJ_DEF) || Object.hasOwn(type, NG_INJECTOR_DEF))
    ? type[NG_INJ_DEF]
    : null;
}
...
var profiler = (event, instance = null, hookOrListener) => {
...
var NgZone = class _NgZone {
  ...
  constructor(options) {
    ...
    if (ngDevMode) {
      this._inner = this._inner.fork(new AsyncStackTaggingZoneSpec('Angular'));
    }
    ...
    this.scheduleInRootZone = scheduleInRootZone;
    forkInnerZoneWithAngularBehavior(this);
  }
  /**
...
function getComponentViewByInstance(componentInstance) {
  const patchedData = readPatchedData(componentInstance);
  let lView;
...
var DehydratedBlockRegistry = class _DehydratedBlockRegistry {
  ...
  cleanup(hydratedBlocks) {
    ...
    for (const blockId of hydratedBlocks) {
...
function stringifyCSSSelector(selector) {
  ...
  while (i < selector.length) {
    const valueOrMarker = selector[i];
...
function applyProjectionRecursive(
  ...
  if (Array.isArray(nodeToProjectOrRNodes)) {
    ...
    const nodeToProject = nodeToProjectOrRNodes;
...
function getTIcu(tView, index) {
  ...
  if (
    ngDevMode &&
    !(Object.hasOwn(value, 'tView') || Object.hasOwn(value, 'currentCaseLViewIndex'))
  ) {
    throwError("We expect to get 'null'|'TIcu'|'TIcuContainer', but got: " + value);
  }
  const tIcu = Object.hasOwn(value, 'currentCaseLViewIndex') ? value : value.value;
...
function ɵɵCopyDefinitionFeature(definition) {
  const superType = getSuperType(definition.type);
  let superDef = void 0;
  if (isComponentDef(definition)) {
...
var ApplicationRef = class _ApplicationRef {
  ...
  synchronizeOnce() {
    ...
    if (this.dirtyFlags & 7) {
      ...
      this.dirtyFlags &= -8;
      this.dirtyFlags |= 8;
      for (const { _lView, notifyErrorHandler } of this.allViews) {
...
function checkStylingMap(keyValueArraySet2, stringParser, value, isClassBased) {
  ...
  if (value !== NO_CHANGE && bindingUpdated(lView, bindingIndex, value)) {
    ...
    if (hasStylingInputShadow(tNode, isClassBased) && !isInHostBindings(tView, bindingIndex)) {
      ...
      const staticPrefix = isClassBased ? tNode.classesWithoutHost : tNode.stylesWithoutHost;
      ngDevMode &&
        isClassBased === false &&
        staticPrefix !== null &&
...
function findStylingValue(tData, tNode, lView, prop, index, isClassBased) {
  ...
  if (tNode !== null) {
    const residual = isClassBased ? tNode.residualClasses : tNode.residualStyles;
    if (residual != null) {
...
function applyIcuSwitchCase(tView, tIcu, lView, value) {
  ...
  const activeCaseIndex = getCurrentICUCaseIndex(tIcu, lView);
...
function i18nUpdateOpCodesToString(opcodes) {
  ...
  while (parser.hasMore()) {
    const mask = parser.consumeNumber();
    const size = parser.consumeNumber();
    const end = parser.i + size;
    const statements = [];
    let statement = '';
    while (parser.i < end) {
      const value = parser.consumeNumberOrString();
...
function icuCreateOpCodesToString(opcodes) {
  ...
  let lastRef = -1;
  while (parser.hasMore()) {
    const value = parser.consumeNumberStringOrMarker();
    if (value === ICU_MARKER) {
...
function parseICUBlock(pattern) {
  ...
  let icuType = 1;
  let mainBinding = 0;
  pattern = pattern.replace(ICU_BLOCK_REGEXP, (str, binding, type) => {
    if (type === 'select') {
      icuType = 0;
    } else {
      icuType = 1;
...
function resolveProvider(
  ...
  if (Array.isArray(provider)) {
    ...
    const lView = getLView();
    const tNode = getCurrentTNode();
    const token = isTypeProvider(provider) ? provider : resolveForwardRef(provider.provide);
...
function patchComponentDefWithScope(componentDef, transitiveScopes) {
  componentDef.directiveDefs = () =>
    Array.from(transitiveScopes.compilation.directives)
      .map((dir) => (Object.hasOwn(dir, NG_COMP_DEF) ? getComponentDef(dir) : getDirectiveDef(dir)))
      .filter((def) => !!def);
...
var __param = (paramIndex, decorator) => (target, key) => {
...
var AuthService = class AuthService2 {
  ...
  updateState(partial) {
    this.stateSubject.next(
      __spreadValues(__spreadValues({}, this.stateSubject.getValue()), partial),
    );
  }
  refreshUser() {
    return __async(this, null, function* () {
...
export { AUTH_CONFIG, AuthService };
