...
function patchProperty(obj, prop, prototype) {
  ...
  desc.get = function () {
    ...
    if (listener) {
      ...
      const value = originalDescGet.call(this);
      if (value) {
...
function patchMacroTask(obj, funcName, metaCreator) {
  ...
  setNative = patchMethod(obj, funcName, (delegate) => (self2, args) => {
...
function patchEventTarget(_global2, api, apis, patchOptions) {
  ...
  const invokeTask = (task, target, event) => {
    if (task.isRemoved) {
      return;
    }
  ...
  function patchEventTargetMethods(obj, patchOptions2) {
    ...
    const customScheduleGlobal = (task) => {
    ...
    const customScheduleNonGlobal = (task) =>
    ...
    const customSchedulePrepend = (task) =>
      nativePrependEventListener.call(
        taskData.target,
        taskData.eventName,
        task.invoke,
        taskData.options,
      );
    const customCancelNonGlobal = (task) =>
      nativeRemoveEventListener.call(task.target, task.eventName, task.invoke, task.options);
    const customSchedule = useGlobalCallback ? customScheduleGlobal : customScheduleNonGlobal;
    const customCancel = useGlobalCallback ? customCancelGlobal : customCancelNonGlobal;
    const compareTaskCallbackVsDelegate = (task, delegate) => {
    ...
    const compare =
      (patchOptions2 == null ? void 0 : patchOptions2.diff) || compareTaskCallbackVsDelegate;
...
function patchEventPrototype(global2, api) {
  ...
  if (Event && Event.prototype) {
    api.patchMethod(Event.prototype, 'stopImmediatePropagation', (delegate) => (self2, args) => {
...
function patchTimer(window2, setName, cancelName, nameSuffix) {
  ...
  setNative = patchMethod(window2, setName, (delegate) => (self2, args) => {
    var _a;
  ...
  clearNative = patchMethod(window2, cancelName, (delegate) => (self2, args) => {
    const id = args[0];
    let task;
    if (isNumber(id)) {
...
function patchBrowser(Zone3) {
  ...
  Zone3.__load_patch('blocking', (global2, Zone4) => {
    ...
    for (let i = 0; i < blockingMethods.length; i++) {
      ...
      patchMethod(global2, name, (delegate, symbol, name2) => {
        return (s, args) => Zone4.current.run(delegate, global2, args, name2);
      });
    }
  });
  ...
  Zone3.__load_patch('XHR', (global2, Zone4) => {
    ...
    function patchXHR(window2) {
      ...
      const openNative = patchMethod(XMLHttpRequestPrototype, 'open', () => (self2, args) => {
      ...
      const sendNative = patchMethod(XMLHttpRequestPrototype, 'send', () => (self2, args) => {
      ...
      const abortNative = patchMethod(XMLHttpRequestPrototype, 'abort', () => (self2, args) => {
  ...
  Zone3.__load_patch('ZoneAwarePromise', (global2, Zone4, api) => {
      ...
      if (promise[symbolState] === UNRESOLVED) {
        ...
        if (
          ...
          if (state === REJECTED && value instanceof Error) {
            const trace =
              Zone4.currentTask && Zone4.currentTask.data && Zone4.currentTask.data[creationTrace];
            if (trace) {
    ...
    class ZoneAwarePromise {
      ...
      static resolve(value) {
        ...
        return resolvePromise(new ZoneAwarePromise(null), RESOLVED, value);
      }
      static reject(error) {
        return resolvePromise(new ZoneAwarePromise(null), REJECTED, error);
      }
      ...
      static race(values) {
        let resolve;
        let reject;
        const promise = new ZoneAwarePromise((res, rej) => {
          resolve = res;
          reject = rej;
        });
        ...
        for (let value of values) {
          if (!isThenable(value)) {
            value = ZoneAwarePromise.resolve(value);
          }
      ...
      static allSettled(values) {
        const P =
          ZoneAwarePromise && ZoneAwarePromise.prototype instanceof ZoneAwarePromise
            ? ZoneAwarePromise
            : ZoneAwarePromise;
      ...
      constructor(executor) {
        ...
        this[symbolState] = UNRESOLVED;
        this[symbolValue] = [];
        try {
          const onceWrapper = once();
          executor &&
            executor(
              onceWrapper(makeResolver(this, RESOLVED)),
              onceWrapper(makeResolver(this, REJECTED)),
            );
        } catch (error) {
    ...
    function zoneify(fn) {
      return (self2, args) => {
        const resultPromise = fn.apply(self2, args);
...
function patchCallbacks(api, target, targetName, method, callbacks) {
  ...
  target[method] = (name, opts, options) => {
