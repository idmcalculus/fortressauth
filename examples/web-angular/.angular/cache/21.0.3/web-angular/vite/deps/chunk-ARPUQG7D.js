...
var profiler = (event, instance = null, eventFn) => {
...
function lookupTokenUsingNodeInjector(tNode, lView, token, flags, notFoundValue) {
  ...
  if (typeof bloomHash === 'function') {
    ...
    const hostTElementNode = flags & 1 ? lView[DECLARATION_COMPONENT_VIEW][T_HOST] : null;
...
var sharedMapFunction = (rEl, jsActionMap) => {
  const el = rEl;
  const blockName = el.getAttribute(DEFER_BLOCK_SSR_ID_ATTRIBUTE) ?? '';
...
function processBlockData(injector) {
  ...
  const blockDetails = /* @__PURE__ */ new Map();
...
function verifySsrContentsIntegrity(doc) {
  ...
  const endOfHead = skipTextNodes(doc.head.lastChild);
...
function determineLongestAnimationFromElementAnimations(el, animationsMap, animations) {
  ...
  for (const animation of animations) {
    ...
    const duration = (timing?.delay ?? 0) + animDuration;
    let propertyName;
    let animationName;
...
var ApplicationRef = class _ApplicationRef {
  ...
  synchronizeOnce() {
    ...
    if (this.dirtyFlags & 7) {
      ...
      this.dirtyFlags &= ~7;
      this.dirtyFlags |= 8;
      for (const { _lView } of this.allViews) {
...
function processAndInitTriggers(injector, blockData, nodes) {
  ...
  for (const [blockId, blockSummary] of blockData) {
...
function replayQueuedBlockEvents(hydratedBlocks) {
  ...
  blockEventQueue = [];
  for (const { event, currentTarget } of queue) {
...
function serializeHydrateTriggers(triggerMap) {
  ...
  const triggers = [];
  for (const [trigger, details] of triggerMap) {
...
//# sourceMappingURL=chunk-ARPUQG7D.js.map
