...
import { __async, __spreadProps, __spreadValues, Subject } from './chunk-AWBSFIDB.js';
...
function _stripOrigin(baseHref) {
  const isAbsoluteUrl2 = /^(https?:)?\/\//.test(baseHref);
  if (isAbsoluteUrl2) {
...
function dateStrGetter(name, width, form = FormStyle.Format, extended = false) {
  return (date, locale) => getDateTranslation(date, locale, name, width, form, extended);
}
...
function roundNumber(parsedNumber, minFrac, maxFrac) {
  ...
  const carry = digits.reduceRight((carry2, d, i, digits2) => {
    d = d + carry2;
...
var NgOptimizedImage = class _NgOptimizedImage {
  ...
  callImageLoader(configWithoutCustomParams) {
    const augmentedConfig = configWithoutCustomParams;
...
function assertPlaceholderDimensions(dir, imgElement) {
  ...
  const renderedWidth = parseFloat(computedStyle.getPropertyValue('width'));
  const renderedHeight = parseFloat(computedStyle.getPropertyValue('height'));
...
//# sourceMappingURL=chunk-QUN7ZOPQ.js.map
