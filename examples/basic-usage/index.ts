#!/usr/bin/env npx tsx
import type { Database } from '@fortressauth/adapter-sql';
...
// Simple mock email provider for this example
const mockEmailProvider: EmailProviderPort = {
  sendVerificationEmail: async (_email: string, _link: string) => {
    console.log('📧 [Mock] Verification email would be sent');
  },
  sendPasswordResetEmail: async (_email: string, _link: string) => {
...
async function main(): Promise<void> {
  ...
  const fortress = new FortressAuth(
    new SqlAdapter(db, { dialect: 'sqlite' }),
